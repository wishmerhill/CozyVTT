...
describe('DiceMacroManager', () => {
  ...
  it('says so plainly when nothing is saved yet', async () => {
    renderManager();
    expect(await screen.findByText(/non hai ancora salvato nulla/i)).toBeInTheDocument();
  });
  ...
  it('pre-fills the expression from the dice panel, so it is not retyped', async () => {
    ...
    expect(screen.getByLabelText('Espressione del tiro salvato')).toHaveValue('2d6+3');
  });
  ...
  it('saves a new macro and tells the panel', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nome del tiro salvato'), { target: { value: 'Wild magic' } });
    fireEvent.change(screen.getByLabelText('Espressione del tiro salvato'), { target: { value: '1d100' } });
  ...
  it('will not submit without both a name and an expression', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Nome del tiro salvato'), { target: { value: 'Nameless' } });
    expect(add).toBeDisabled();

    fireEvent.change(screen.getByLabelText('Espressione del tiro salvato'), { target: { value: '1d20' } });
  ...
  describe('when the server refuses an expression', () => {
    it('shows the reason it gave, rather than a vaguer one of its own', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Nome del tiro salvato'), { target: { value: 'Greedy' } });
      fireEvent.change(screen.getByLabelText('Espressione del tiro salvato'), { target: { value: '101d20' } });
      fireEvent.click(screen.getByRole('button', { name: /aggiungi/i }));
    ...
    it('keeps what was typed so it can be corrected', async () => {
      ...
      fireEvent.change(screen.getByLabelText('Nome del tiro salvato'), { target: { value: 'Typo' } });
      fireEvent.change(screen.getByLabelText('Espressione del tiro salvato'), { target: { value: '2d6+' } });
      fireEvent.click(screen.getByRole('button', { name: /aggiungi/i }));

      await screen.findByRole('alert');
      expect(screen.getByLabelText('Espressione del tiro salvato')).toHaveValue('2d6+');
      expect(screen.getByLabelText('Nome del tiro salvato')).toHaveValue('Typo');
    });
  });
  ...
  describe('editing', () => {
    ...
    it('leaves an edited macro exactly where it was', async () => {
      ...
      fireEvent.click(screen.getByLabelText('Modifica First'));
      fireEvent.change(screen.getByLabelText('Nome per First'), { target: { value: 'First edited' } });
      fireEvent.click(screen.getByLabelText('Salva le modifiche a First'));
    ...
    it('can be cancelled without saving', async () => {
      ...
      fireEvent.click(screen.getByLabelText('Modifica Second'));
      fireEvent.change(screen.getByLabelText('Nome per Second'), { target: { value: 'Discarded' } });
    ...
    it('refuses to store an edit the server rejects, and says why', async () => {
      ...
      fireEvent.click(screen.getByLabelText('Modifica First'));
      fireEvent.change(screen.getByLabelText('Espressione per First'), { target: { value: '2d6+' } });
      fireEvent.click(screen.getByLabelText('Salva le modifiche a First'));
  ...
  describe('deleting', () => {
    ...
    it('asks before deleting', async () => {
      ...
      fireEvent.click(screen.getByLabelText('Elimina Doomed'));

      expect(await screen.findByText(/elimina tiro salvato/i)).toBeInTheDocument();
