...
describe('AtmospherePanel audio list', () => {
  ...
  it('names each scope as the rest of the app does', async () => {
    ...
    expect(screen.getByText(/Personali/)).toBeInTheDocument();
    expect(screen.getByText(/Globali/)).toBeInTheDocument();
    expect(screen.getByText(/Campagna/)).toBeInTheDocument();
  });
});
