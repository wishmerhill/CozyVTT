...
// The test setup initializes i18next with the Italian resources (see
// src/test/setup.ts), so assertions match the `it` translations rather than
...
const genericLabel = 'Tiro Personalizzato';
...
describe('CustomRollFooter', () => {
  ...
  it('falls back to a generic label when none is given', () => {
    ...
    fireEvent.click(screen.getByRole('button'));
    expect(onRoll).toHaveBeenCalledWith('1d20', genericLabel);
  });
  ...
  it('refuses an expression the dice parser would reject', () => {
    ...
    expect(screen.getByText('Espressione di dado non valida')).toBeInTheDocument();
  });
  ...
  it('asks for an expression rather than rolling nothing', () => {
    ...
    fireEvent.click(screen.getByRole('button'));
    expect(onRoll).not.toHaveBeenCalled();
    expect(screen.getByText("Inserisci un'espressione di dado")).toBeInTheDocument();
  });
  ...
  it('rolls on Enter, so it can be used without reaching for the button', () => {
    ...
    expect(onRoll).toHaveBeenCalledWith('1d20+5', genericLabel);
  });
});
