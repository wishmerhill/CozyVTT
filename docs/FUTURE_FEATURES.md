...
- **Metric/imperial measurement system** — grid distance (`feetPerSquare`) is imperial-only, hardcoded "ft" labels throughout the ruler, AoE templates, and map setup UI. Needs a per-instance default (chosen in the Setup Wizard) with a per-map override. Full analysis and 4-phase plan already scoped: see `docs/MEASUREMENT_SYSTEM_PLAN.md`.
